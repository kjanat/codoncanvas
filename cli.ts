...
program
  ...
  .description('Validate a single .genome file for frame alignment and structure')
  ...
  .action((file: string, options: { verbose?: boolean; json?: boolean }) => {
    try {
      ...
      if (options.json) {
        ...
        if (allErrors.length === 0) {
          ...
          if (options.verbose) {
            ...
            console.log(chalk.gray(`  Bases: ${source.replace(/\s+/g, '').replace(/;.*/g, '').length}`));
          }
        } else {
          ...
          allErrors.forEach(err => {
            ...
            console.log(color(`  ${icon} ${err.message} (position ${err.position})`));
            if (err.fix) {
      ...
      process.exit(allErrors.filter(e => e.severity === 'error').length > 0 ? 1 : 0);
    } catch (error) {
...
program
  ...
  .description('Validate multiple .genome files (supports glob patterns like "examples/*.genome")')
  .option('--fix', 'Automatically fix frame alignment issues (not implemented yet)')
  .option('--json', 'Output results as JSON')
  .action((pattern: string, options: { fix?: boolean; json?: boolean }) => {
    ...
    const results: Array<{ file: string; valid: boolean; errors: ParseError[] }> = [];
    const lexer = new CodonLexer();
    ...
    for (const file of files) {
      try {
        ...
        results.push({
          file,
          valid: false,
          errors: [{ message: `Failed to read: ${error}`, position: 0, severity: 'error' }]
        });
      }
    }
...
program
  ...
  .description('Check similarity between two genomes (useful for plagiarism detection)')
  .option('-t, --threshold <number>', 'Similarity threshold (0-1, default: 0.8)', '0.8')
  .action((file1: string, file2: string, options: { threshold: string }) => {
    try {
      const source1 = readFileSync(file1, 'utf-8').replace(/\s+/g, '').replace(/;.*/g, '');
      const source2 = readFileSync(file2, 'utf-8').replace(/\s+/g, '').replace(/;.*/g, '');
      ...
      console.log(`  Longest common substring: ${lcs.length} bases (${(lcsRatio * 100).toFixed(1)}%)`);
      ...
      if (isSimilar) {
        ...
        if (lcs.length > 30) {
          console.log(chalk.gray(`  Common sequence: ${lcs.substring(0, 60)}...`));
        }
      } else {
        console.log(chalk.green(`✓ Files are DIFFERENT (likely independent work)`));
      }
  ...
  .action((file: string) => {
    try {
      ...
      tokens.forEach(token => {
        ...
        const family = firstBase === 'A' ? 'Stack/Control' :
                      firstBase === 'C' ? 'Shapes/Utility' :
                      firstBase === 'G' ? 'Shapes/Stack' :
                      'Transform/Color';
      ...
      Object.entries(opcodeCounts)
        ...
        .forEach(([family, count]) => {
          console.log(`  ${family}: ${count} (${((count / tokens.length) * 100).toFixed(1)}%)`);
        });
      console.log();
      console.log(chalk.bold('Complexity:'));
      console.log(`  Avg codons per line: ${(tokens.length / lines).toFixed(1)}`);
      console.log(`  Documentation ratio: ${((comments / (lines + comments)) * 100).toFixed(1)}%`);
    } catch (error) {
      console.error(chalk.red(`Error: ${error}`));
      process.exit(1);
    }
