<script lang="ts">
  import { loadEngine, type Engine } from "./lib/engine/index.js";
  ...
  <nav class="main-nav">
    {#each NAV as item (item.route)}
      <button
        ...
        onclick={() => router.go(item.route)}>{item.label}</button
      >
    {/each}
  </nav>
</header>
...
<main>
  {#if loadError}
    <div class="panel">
      ...
      <p class="muted">
        Build the WASM module first: <code class="mono">npm run build:wasm</code>.
      </p>
    </div>
  {:else if !engine}
    <div class="panel"><p class="muted">Loading the WebAssembly engine…</p></div>
