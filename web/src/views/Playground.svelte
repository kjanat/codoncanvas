<script lang="ts">
  import type { Engine, RunResult, Diagnostic, Example } from "../lib/engine/index.js";
  ...
  const shownCount = $derived(
    showAll ? result.commands.length : result.steps[stepValue]?.command_count ?? 0,
  );
  const currentStep = $derived(showAll ? null : result.steps[stepValue] ?? null);
