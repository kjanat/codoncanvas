...
<div class="panel">
	...
	<p>
		...
		map to the same operation, so mutations behave like real genetics —
	...
	<ul>
		<li>
			The entire engine — lexer, codon map, stack VM, and mutation tools —
			was rewritten in idiomatic Rust (edition 2024) and compiled to
			WebAssembly. It runs the same code natively (for tests) and in your
			browser.
		</li>
		<li>
		...
		<li>
			...
			<span class="mono">TRANSLATE</span> could only move right and down. The
			new engine treats translation offsets as <em>signed around the midpoint
				32</em>:
			<span class="mono">offset = ((v − 32) / 64) × width</span>. So
			<span class="mono">32</span> means "stay put", values below 32 move
			left/up, and values above 32 move right/down. Shape sizes keep the
			original unsigned mapping, since a negative radius is meaningless. See the
	...
	<p class="muted">
		...
		ROTATE, SCALE, COLOR. Stack: PUSH, DUP, POP, SWAP. Arithmetic: ADD, SUB,
		MUL, DIV. Comparison: EQ, LT. Control: START, STOP, LOOP. State: SAVE_STATE,
