...
class Stats {
  ...
  static sd(values: number[], sample = true): number {
    ...
    const variance = values.reduce((sum, val) => sum + Math.pow(val - m, 2), 0) /
  ...
  static pairedTTest(pre: number[], post: number[]): TTestResult {
    ...
    const sdDiff = this.sd(differences);
    const seMean = sdDiff / Math.sqrt(n);

    const t = meanDiff / seMean;
    const df = n - 1;
    const p = this.tTestPValue(Math.abs(t), df);
  ...
  static independentTTest(group1: number[], group2: number[]): TTestResult {
    ...
    const sd1 = this.sd(group1);
    const sd2 = this.sd(group2);
  ...
  static tTestPValue(t: number, df: number): number {
    ...
    const ratio = (t * t) / df;
    const p_one_tail = 0.5 * Math.pow(1 + ratio, -(df + 1) / 2);
  ...
  static powerAnalysis(
    ...
    const z_beta = power === 0.80 ? 0.84 : (power === 0.90 ? 1.28 : 0.52);

    const n_per_group = Math.ceil(2 * Math.pow(z_alpha + z_beta, 2) / Math.pow(effect_size, 2));

    // Account for 20% attrition
    const inflated_n = Math.ceil(n_per_group / 0.80);
...
class ResearchAnalyzer {
  ...
  static analyzePrePost(data: StudentData[]): void {
    console.log('\n╔══════════════════════════════════════════════════════════════╗');
    console.log('║       CodonCanvas Pre-Post Analysis (Pilot Study)           ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    this.printDescriptives(preStats);
    console.log('\nPost-test:');
    this.printDescriptives(postStats);

    // Paired t-test
    console.log('\n\n📈 PAIRED T-TEST (Pre vs. Post)\n');
    const tResult = Stats.pairedTTest(preScores, postScores);

    console.log(`t(${tResult.df}) = ${tResult.t.toFixed(3)}, p = ${this.formatPValue(tResult.p)}`);
    console.log(`Mean difference: ${tResult.mean_diff.toFixed(2)} (95% CI: [${tResult.ci_lower.toFixed(2)}, ${tResult.ci_upper.toFixed(2)}])`);
    console.log(`Cohen's d: ${tResult.cohens_d.toFixed(3)} (${Stats.interpretEffectSize(tResult.cohens_d)})`);
    ...
    if (withDelayed.length > 5) {
      ...
      this.analyzeRetention(withDelayed);
    }
  ...
  static analyzeRCT(
    ...
    console.log('\n╔══════════════════════════════════════════════════════════════╗');
    console.log('║         CodonCanvas RCT Analysis (Treatment vs. Control)     ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    console.log(`Treatment pre-test: M = ${Stats.mean(treatmentPre).toFixed(2)}, SD = ${Stats.sd(treatmentPre).toFixed(2)}`);
    console.log(`Control pre-test:   M = ${Stats.mean(controlPre).toFixed(2)}, SD = ${Stats.sd(controlPre).toFixed(2)}`);
    console.log(`t(${preEquiv.df}) = ${preEquiv.t.toFixed(3)}, p = ${this.formatPValue(preEquiv.p)}`);

    if (preEquiv.p >= 0.05) {
      console.log('✅ Groups are equivalent at baseline (good randomization)');
    } else {
      console.log('⚠️  Groups differ at baseline (consider ANCOVA with pre-test covariate)');
    }
    ...
    console.log(`Treatment post-test: M = ${Stats.mean(treatmentPost).toFixed(2)}, SD = ${Stats.sd(treatmentPost).toFixed(2)}`);
    console.log(`Control post-test:   M = ${Stats.mean(controlPost).toFixed(2)}, SD = ${Stats.sd(controlPost).toFixed(2)}`);
    console.log(`\nt(${postResult.df}) = ${postResult.t.toFixed(3)}, p = ${this.formatPValue(postResult.p)}`);
    console.log(`Mean difference: ${postResult.mean_diff.toFixed(2)} (95% CI: [${postResult.ci_lower.toFixed(2)}, ${postResult.ci_upper.toFixed(2)}])`);
    console.log(`Cohen's d: ${postResult.cohens_d.toFixed(3)} (${Stats.interpretEffectSize(postResult.cohens_d)})`);

    if (postResult.p < 0.05) {
      console.log('\n✅ SIGNIFICANT treatment effect detected (p < 0.05)');
      console.log(`   CodonCanvas shows ${Stats.interpretEffectSize(postResult.cohens_d)} effect size`);
    } else {
    ...
    // Gain scores
    console.log('\n\n📈 GAIN SCORE ANALYSIS\n');
    const treatmentGains = treatmentData.map(d => d.posttest_total - d.pretest_total);
    const controlGains = controlData.map(d => d.posttest_total - d.pretest_total);

    const gainResult = Stats.independentTTest(treatmentGains, controlGains);
    console.log(`Treatment gain: M = ${Stats.mean(treatmentGains).toFixed(2)}, SD = ${Stats.sd(treatmentGains).toFixed(2)}`);
    console.log(`Control gain:   M = ${Stats.mean(controlGains).toFixed(2)}, SD = ${Stats.sd(controlGains).toFixed(2)}`);
    console.log(`t(${gainResult.df}) = ${gainResult.t.toFixed(3)}, p = ${this.formatPValue(gainResult.p)}`);
  ...
  static analyzeSubscales(data: StudentData[]): void {
    const mutationTypes = ['silent', 'missense', 'nonsense', 'frameshift', 'indel'];

    console.log('Mutation Type | Pre M(SD)  | Post M(SD) | Gain M(SD) | Cohen\'s d');
    ...
    for (const type of mutationTypes) {
      ...
      console.log(
        `${type.padEnd(13)} | ${preMean}(${preSD}) | ${postMean}(${postSD}) | ${gainMean}(${gainSD}) | ${d.toFixed(3)} (${Stats.interpretEffectSize(d)})`
      );
    }
  }
  ...
  static analyzeRetention(data: StudentData[]): void {
    ...
    console.log(`Immediate post-test: M = ${postMean.toFixed(2)}, SD = ${Stats.sd(postScores).toFixed(2)}`);
    console.log(`Delayed post-test:   M = ${delayedMean.toFixed(2)}, SD = ${Stats.sd(delayedScores).toFixed(2)}`);
    console.log(`Retention rate:      ${retention.toFixed(1)}% of immediate post-test score`);

    const retentionTest = Stats.pairedTTest(postScores, delayedScores);
    console.log(`t(${retentionTest.df}) = ${retentionTest.t.toFixed(3)}, p = ${this.formatPValue(retentionTest.p)}`);
   ...
   */
  static generateTable(data: StudentData[], group: 'treatment' | 'control' | 'all'): void {
    console.log('\n📋 PUBLICATION TABLE (Markdown format)\n');
    console.log('```markdown');
    console.log('| Variable | N | Pre M(SD) | Post M(SD) | Gain M(SD) | t | df | p | d |');
    console.log('|----------|---|-----------|------------|------------|---|----|----|---|');
    ...
    console.log(`| Total Score | ${data.length} | ${Stats.mean(pre).toFixed(2)}(${Stats.sd(pre).toFixed(2)}) | ${Stats.mean(post).toFixed(2)}(${Stats.sd(post).toFixed(2)}) | ${Stats.mean(gains).toFixed(2)}(${Stats.sd(gains).toFixed(2)}) | ${result.t.toFixed(2)} | ${result.df} | ${this.formatPValue(result.p)} | ${result.cohens_d.toFixed(2)} |`);
    console.log('```\n');
  }
}
...
function parseCSV(filepath: string): StudentData[] {
  ...
  return lines.slice(1).map(line => {
    ...
    headers.forEach((header, i) => {
      ...
      if (header.includes('test_') || header === 'mtt_score' || header === 'gpa' ||
          header.includes('imi_') || header === 'prior_programming') {
...
function main(): void {
  ...
  if (args.includes('--power-analysis')) {
    ...
    const power = powerIdx !== -1 ? parseFloat(args[powerIdx + 1]) : 0.80;
  ...
  if (design === 'prepost') {
    ...
    if (controlIdx === -1) {
      ...
      if (treatment.length === 0 || control.length === 0) {
        console.error('Error: For RCT, either provide --control file or include "group" column in data');
        process.exit(1);
      }
...
export { Stats, ResearchAnalyzer, parseCSV, StudentData };
