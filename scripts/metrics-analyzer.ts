...
function generateReport(analyzer: MetricsAnalyzer, outputPath: string): void {
  ...
  report += `Average Session Duration:    ${formatDuration(engagement.avgSessionDuration.mean)}\n`;
  report += `  (SD = ${formatDuration(engagement.avgSessionDuration.sd)}, Range: ${formatDuration(engagement.avgSessionDuration.min)} - ${formatDuration(engagement.avgSessionDuration.max)})\n\n`;
  report += `Total Genomes Created:       ${engagement.totalGenomesCreated}\n`;
  report += `Avg Genomes per Session:     ${engagement.avgGenomesPerSession.mean.toFixed(1)}\n`;
  report += `  (SD = ${engagement.avgGenomesPerSession.sd.toFixed(1)})\n`;
  report += `Genome Execution Rate:       ${engagement.genomesExecutedRate.toFixed(1)}%\n\n`;
  ...
  report += `  Mean:     ${formatDuration(velocity.timeToFirstArtifact.mean)}\n`;
  report += `  Median:   ${formatDuration(velocity.timeToFirstArtifact.median)}\n`;
  report += `  SD:       ${formatDuration(velocity.timeToFirstArtifact.sd)}\n`;
  report += `  Range:    ${formatDuration(velocity.timeToFirstArtifact.min)} - ${formatDuration(velocity.timeToFirstArtifact.max)}\n\n`;
  report += `Learner Distribution:\n`;
  report += `  Fast (<5 min):       ${velocity.fastLearners} (${((velocity.fastLearners / engagement.totalSessions) * 100).toFixed(1)}%)\n`;
  report += `  Moderate (5-15 min): ${velocity.moderateLearners} (${((velocity.moderateLearners / engagement.totalSessions) * 100).toFixed(1)}%)\n`;
  report += `  Slow (>15 min):      ${velocity.slowLearners} (${((velocity.slowLearners / engagement.totalSessions) * 100).toFixed(1)}%)\n`;
  report += `  No Artifact:         ${velocity.noArtifact} (${((velocity.noArtifact / engagement.totalSessions) * 100).toFixed(1)}%)\n\n`;

  // Tool Adoption
  report += '───────────────────────────────────────────────────────────────────\n';
  report += '3. TOOL ADOPTION\n';
  report += '───────────────────────────────────────────────────────────────────\n\n';
  report += formatToolRow('Diff Viewer', tools.diffViewer, engagement.totalSessions);
  report += formatToolRow('Timeline Scrubber', tools.timeline, engagement.totalSessions);
  report += formatToolRow('Evolution Lab', tools.evolution, engagement.totalSessions);
  report += formatToolRow('Assessment System', tools.assessment, engagement.totalSessions);
  report += formatToolRow('Export Features', tools.export, engagement.totalSessions);
  report += '\n';
  ...
  report += '───────────────────────────────────────────────────────────────────\n\n';
  report += `Visual Only:         ${renderMode.visualOnly.percentage.toFixed(1)}% (${renderMode.visualOnly.sessions} executions)\n`;
  report += `Audio Only:          ${renderMode.audioOnly.percentage.toFixed(1)}% (${renderMode.audioOnly.sessions} executions)\n`;
  report += `Multi-Sensory:       ${renderMode.multiSensory.percentage.toFixed(1)}% (${renderMode.multiSensory.sessions} executions)\n\n`;
  ...
  report += formatMutationRow('Silent', mutations.silent, mutations.totalMutations);
  report += formatMutationRow('Missense', mutations.missense, mutations.totalMutations);
  report += formatMutationRow('Nonsense', mutations.nonsense, mutations.totalMutations);
  report += formatMutationRow('Frameshift', mutations.frameshift, mutations.totalMutations);
  report += formatMutationRow('Point', mutations.point, mutations.totalMutations);
  report += formatMutationRow('Insertion', mutations.insertion, mutations.totalMutations);
  report += formatMutationRow('Deletion', mutations.deletion, mutations.totalMutations);
...
function formatToolRow(name: string, data: { users: number; avgUsage: number }, total: number): string {
  const adoption = (data.users / total) * 100;
  return `${name.padEnd(20)} ${adoption.toFixed(1)}% adoption (avg ${data.avgUsage.toFixed(1)} uses/session)\n`;
}

function formatMutationRow(name: string, stats: DescriptiveStats, total: number): string {
  const sum = stats.mean * stats.n;
  const percentage = total > 0 ? (sum / total) * 100 : 0;
  return `  ${name.padEnd(12)} ${percentage.toFixed(1)}% (M=${stats.mean.toFixed(1)}, SD=${stats.sd.toFixed(1)})\n`;
}
...
function generateComparisonReport(
  ...
  for (const comp of comparisons) {
    ...
    report += `Difference:      ${comp.diff > 0 ? '+' : ''}${comp.diff.toFixed(2)} (${comp.percentChange > 0 ? '+' : ''}${comp.percentChange.toFixed(1)}%)\n\n`;
    report += `t-test:          t = ${comp.t.toFixed(3)}, p = ${comp.p.toFixed(4)}\n`;
    report += `Effect Size:     Cohen's d = ${comp.cohensD.toFixed(3)} (${comp.interpretation})\n`;
  ...
  try {
    ...
    if (baselineFile) {
      if (!fs.existsSync(baselineFile)) {
        ...
        const comparisonPath = path.join(outputDir, `${baseName}_comparison.txt`);
...
export { Stats, MetricsAnalyzer, parseCSV, MetricsSession };
